...
class LobbyManager extends EventEmitter {
  ...
  createOpenLobby (lobbyConfig={}) {
    if (!lobbyConfig.playerCountForGame) lobbyConfig.playerCountForGame = 2; // Always set default here for consistent hashes
  ...
  getOpenLobbiesWithExactLobbyConfig(lobbyConfig={}) {
    ...
    if (this.isExactConfigHashingEnabled()) {
      ...
      if (!this.lobbyConfigExactHashes[hashId]) {
        return [];
      }

      let lobbyIdArrMatchingConfig = this.lobbyConfigExactHashes[hashId];
      let matchingLobbies = lobbyIdArrMatchingConfig.map(lobbyId => this.openLobbies[lobbyId]);

      // The lobby && check is because these matchedLobbyIds will include those from runningLobbies.
      // return empty array if none found. only include open lobbies as tested with isFull
      return matchingLobbies.filter(lobby => lobby && !lobby.isFull()) || [];
    } else {
  ...
  getFirstOpenLobbyWithCriteria (lobbyConfig={}) {
    if (this.isConfigIndexingEnabled()) {
      return this[_getFirstOpenLobby_withIndexing](lobbyConfig);
    } else {
      return this[_getFirstOpenLobby_withoutIndexing](lobbyConfig);
    }
  }

  getAllOpenLobbiesWithCriteria (lobbyConfig={}) {
    if (this.isConfigIndexingEnabled()) {
      return this[_getAllOpenLobbiesWithCriteria_withIndexing](lobbyConfig);
    } else {
      return this[_getAllOpenLobbiesWithCriteria_withoutIndexing](lobbyConfig);
    }
  }
  // More efficient on memory, less efficient on CPU
  [_getFirstOpenLobby_withoutIndexing] (lobbyConfig={}) {
    if (!lobbyConfig.playerCountForGame) lobbyConfig.playerCountForGame = 2; // Always set default here for consistent hashes
    ...
    for (let lobbyId in this.openLobbies) {
      ...
      let isMatch = true;
      for (let i in lobbyConfig) {
        if (lobby.lobbyConfig[i] != lobbyConfig[i]) {
          isMatch = false;
  ...
  [_getFirstOpenLobby_withIndexing] (lobbyConfig={}) {
    return this[_getAllOpenLobbiesWithCriteria_withIndexing](lobbyConfig)[0];
  }

  // More CPU efficient, slightly less memory-efficient
  [_getAllOpenLobbiesWithCriteria_withIndexing] (lobbyConfig) {
    if (!lobbyConfig.playerCountForGame) lobbyConfig.playerCountForGame = 2; // Always set default here for consistent hashes
  ...
  [_getAllOpenLobbiesWithCriteria_withoutIndexing] (lobbyConfig={}) {
    if (!lobbyConfig.playerCountForGame) lobbyConfig.playerCountForGame = 2; // Always set default here for consistent hashes
  ...
  getTotalPlayerCount () {
    return {
      openLobbyPlayers: Object.keys(this.openLobbies).map(
        ...
        ).reduce((acc, value, index, arr) => acc + value, 0),
  ...
  getLobbyBySocket (socket) {
    ...
    let relevantRooms = rooms.filter(room =>
      room !== sockId && room !== '' && room.indexOf(this.socketRoomPrefix) == 0 // Enforce prefix 
    );
